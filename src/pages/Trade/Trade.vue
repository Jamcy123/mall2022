    ...
    <div class="content">
      ...
      <div class="detail">
        ...
        <ul class="list clearFix" v-for="detail in orderInfo.detailArrayList" :key="detail.skuId">
          <li>
            <img :src="detail.imgUrl" style="width:80px" />
          </li>
          <li>
            <p>
              {{ detail.skuName }}
...
import { reqSubmitOrder } from '@/network';
...
export default {
  ...
  methods: {
    changeDefault(address) {
      ...
      address.isDefault = '1';
      const { consignee, userAddress, phoneNum } = address;
      this.user = { consignee, userAddress, phoneNum };
    },
    async submitOrder() {
      const { tradeNo } = this.orderInfo;
      const data = {
        consignee: this.user.consignee,
        consigneeTel: this.user.phoneNum,
        deliveryAddress: this.user.userAddress,
        paymentWay: 'ONLINE',
        orderComment: this.msg,
        orderDetailList: this.orderInfo.detailArrayList
      };
      await reqSubmitOrder(tradeNo, data)
        .then(result => {
